...
class JaffGen:
    ...
    *CLI arg* takes priority over *config file* value; absent both, the
    ...
    def __init__(self, args: SimpleNamespace):
        ...
        Receives parsed arguments from the Typer entry point, resolves
        configuration from a ``jaffgen.toml`` file when present, loads the
        chemical reaction network, and writes generated source files to the
        output directory.
        ...
        args : SimpleNamespace
            ...
            (``network``, ``config``, ``indir``, ``files``, ``template``,
            ``outdir``, ``lang``, ``network_config``, ``funcfile``, ``label``,
            ``replace_nH``, ``errors``).
